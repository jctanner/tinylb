...
def is_gateway_programmed(k8s_client, namespace, gateway_name):
    ...
    print(f"🔍 [gateway_helpers.is_gateway_programmed] Checking if gateway '{gateway_name}' is programmed in namespace '{namespace}'")
...
def get_gateway_address(k8s_client, namespace, gateway_name):
    ...
    print(f"🔍 [gateway_helpers.get_gateway_address] Getting address for gateway '{gateway_name}' in namespace '{namespace}'")
    gateway = get_gateway(k8s_client, namespace, gateway_name)
    if not gateway:
        return None
...
def wait_for_gateway_programmed(k8s_client, namespace, gateway_name, timeout=120):
    ...
    print(f"⏳ [gateway_helpers.wait_for_gateway_programmed] Waiting for gateway '{gateway_name}' to be programmed (timeout: {timeout}s)")
...
def create_test_gateway(k8s_client, namespace, gateway_name, gateway_class="istio"):
    ...
    print(f"➕ [gateway_helpers.create_test_gateway] Creating test gateway '{gateway_name}' with class '{gateway_class}' in namespace '{namespace}'")
    gateway_spec = {
...
def create_test_httproute(
    ...
    print(f"➕ [gateway_helpers.create_test_httproute] Creating test httproute '{httproute_name}' for gateway '{gateway_name}' -> backend '{backend_service_name}' in namespace '{namespace}'")
    httproute_spec = {
        "apiVersion": "gateway.networking.k8s.io/v1beta1",
        "kind": "HTTPRoute",
...
def create_test_backend_service(k8s_client, namespace, service_name="echo-service"):
    ...
    print(f"➕ [gateway_helpers.create_test_backend_service] Creating test backend service '{service_name}' in namespace '{namespace}'")
    # Try relative import first (for pytest), then absolute import (for standalone)
    try:
...
def wait_for_istio_service_creation(k8s_client, namespace, gateway_name, timeout=120):
    ...
    print(f"⏳ [gateway_helpers.wait_for_istio_service_creation] Waiting for Istio service '{gateway_name}-istio' for gateway '{gateway_name}' (timeout: {timeout}s)")
...
def get_gateway_service_name(gateway_name, gateway_class="istio"):
    ...
    print(f"🔍 [gateway_helpers.get_gateway_service_name] Getting expected service name for gateway '{gateway_name}' with class '{gateway_class}'")
...
def get_gateway_conditions(k8s_client, namespace, gateway_name):
    ...
    print(f"🔍 [gateway_helpers.get_gateway_conditions] Getting conditions for gateway '{gateway_name}' in namespace '{namespace}'")
    gateway = get_gateway(k8s_client, namespace, gateway_name)
    if not gateway:
        return []
...
def print_gateway_status(k8s_client, namespace, gateway_name):
    ...
    print(f"📊 [gateway_helpers.print_gateway_status] Printing status for gateway '{gateway_name}' in namespace '{namespace}'")
    gateway = get_gateway(k8s_client, namespace, gateway_name)
    if not gateway:
...
def verify_gateway_route_integration(k8s_client, namespace, gateway_name):
    ...
    print(f"🔍 [gateway_helpers.verify_gateway_route_integration] Verifying integration between gateway '{gateway_name}' and its route in namespace '{namespace}'")
    # Try relative import first (for pytest), then absolute import (for standalone)
    try:
