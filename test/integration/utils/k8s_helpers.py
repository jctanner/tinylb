...
def get_service(k8s_client, namespace, name):
    ...
    print(f"🔍 [k8s_helpers.get_service] Getting service '{name}' in namespace '{namespace}'")
    try:
        return k8s_client['core'].read_namespaced_service(name=name, namespace=namespace)
...
def get_deployment(k8s_client, namespace, name):
    ...
    print(f"🔍 [k8s_helpers.get_deployment] Getting deployment '{name}' in namespace '{namespace}'")
    try:
        return k8s_client['apps'].read_namespaced_deployment(name=name, namespace=namespace)
    except ApiException as e:
        if e.status == 404:
            return None
        raise
...
def get_route(k8s_client, namespace, name):
    ...
    print(f"🔍 [k8s_helpers.get_route] Getting route '{name}' in namespace '{namespace}'")
    try:
...
def get_gateway(k8s_client, namespace, name):
    ...
    print(f"🔍 [k8s_helpers.get_gateway] Getting gateway '{name}' in namespace '{namespace}'")
    try:
        return k8s_client['custom'].get_namespaced_custom_object(
...
def get_httproute(k8s_client, namespace, name):
    ...
    print(f"🔍 [k8s_helpers.get_httproute] Getting httproute '{name}' in namespace '{namespace}'")
    try:
        return k8s_client['custom'].get_namespaced_custom_object(
            group="gateway.networking.k8s.io",
            version="v1beta1",
...
def create_service(k8s_client, namespace, service_spec):
    """Create a service"""
    service_name = service_spec.metadata.name if hasattr(service_spec, 'metadata') else 'unknown'
    print(f"➕ [k8s_helpers.create_service] Creating service '{service_name}' in namespace '{namespace}'")
...
def create_deployment(k8s_client, namespace, deployment_spec):
    """Create a deployment"""
    deployment_name = deployment_spec.metadata.name if hasattr(deployment_spec, 'metadata') else 'unknown'
    print(f"➕ [k8s_helpers.create_deployment] Creating deployment '{deployment_name}' in namespace '{namespace}'")
...
def create_gateway(k8s_client, namespace, gateway_spec):
    ...
    print(f"➕ [k8s_helpers.create_gateway] Creating gateway '{gateway_name}' in namespace '{namespace}'")
...
def create_httproute(k8s_client, namespace, httproute_spec):
    ...
    print(f"➕ [k8s_helpers.create_httproute] Creating httproute '{httproute_name}' in namespace '{namespace}'")
    return k8s_client['custom'].create_namespaced_custom_object(
        group="gateway.networking.k8s.io",
        version="v1beta1",
        namespace=namespace,
...
def wait_for_service_external_ip(k8s_client, namespace, service_name, timeout=120):
    ...
    print(f"⏳ [k8s_helpers.wait_for_service_external_ip] Waiting for service '{service_name}' to get external IP (timeout: {timeout}s)")
...
def wait_for_deployment_ready(k8s_client, namespace, deployment_name, timeout=120):
    ...
    print(f"⏳ [k8s_helpers.wait_for_deployment_ready] Waiting for deployment '{deployment_name}' to be ready (timeout: {timeout}s)")
    start_time = time.time()
    while time.time() - start_time < timeout:
...
def wait_for_route_creation(k8s_client, namespace, route_name, timeout=120):
    ...
    print(f"⏳ [k8s_helpers.wait_for_route_creation] Waiting for route '{route_name}' to be created (timeout: {timeout}s)")
    start_time = time.time()
    while time.time() - start_time < timeout:
...
def list_services_by_type(k8s_client, namespace, service_type="LoadBalancer"):
    ...
    print(f"📋 [k8s_helpers.list_services_by_type] Listing services of type '{service_type}' in namespace '{namespace}'")
...
def list_routes_with_label(k8s_client, namespace, label_selector):
    ...
    print(f"📋 [k8s_helpers.list_routes_with_label] Listing routes with label selector '{label_selector}' in namespace '{namespace}'")
    try:
...
def patch_service_status(k8s_client, namespace, service_name, status_patch):
    ...
    print(f"🔧 [k8s_helpers.patch_service_status] Patching status for service '{service_name}' in namespace '{namespace}'")
...
def get_pod_logs(k8s_client, namespace, pod_name, container=None):
    ...
    print(f"📄 [k8s_helpers.get_pod_logs] Getting logs from pod '{pod_name}'{container_info} in namespace '{namespace}'")
    try:
...
def list_pods_by_label(k8s_client, namespace, label_selector):
    ...
    print(f"📋 [k8s_helpers.list_pods_by_label] Listing pods with label selector '{label_selector}' in namespace '{namespace}'")
