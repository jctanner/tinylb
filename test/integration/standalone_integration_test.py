...
def check_prerequisites(k8s_client):
    ...
    try:
        deployments = k8s_client["apps"].list_namespaced_deployment(namespace="tinylb-system")
        tinylb_found = False
        ...
        if not tinylb_found:
            print("   ⚠️  TinyLB deployment not found (this is expected if TinyLB is not deployed)")
    ...
    try:
        ...
        if len(gateway_crds) >= 2:
            ...
            print(f"   ❌ Found only {len(gateway_crds)} Gateway API CRDs (need at least 2)")
            success = False
        ...
        for attempt in range(30):
            ...
            if service.status.load_balancer.ingress:
                ...
                if external_ip == route_host:
                    ...
                    print(f"   ❌ Service external IP ({external_ip}) doesn't match route host ({route_host})")
                    return False
            else:
                print(f"   ⏳ Waiting for service external IP... (attempt {attempt + 1}/30)")
                time.sleep(1)
        else:
